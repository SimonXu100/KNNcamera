...
//extern NSString * const kSearchTextDidChangeNotification;
extern NSString * const kStartSearchNotification;
...
@property (nonatomic, strong) KNResultViewController *searchResultController;
...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT) style:UITableViewStyleGrouped];
...
- (KNResultViewController *)searchResultController
{
    if (!_searchResultController) {
        _searchResultController = [[KNResultViewController alloc] init];
    }
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.baseSearchTableView.frame = CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT);
...
- (void)setup
{
    ...
    // 设置搜索结果显示模式
    self.searchResultShowMode = PYSearchResultShowModePush;
    // 显示搜索建议
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:kSearchTextDidChangeNotification object:nil];
}
